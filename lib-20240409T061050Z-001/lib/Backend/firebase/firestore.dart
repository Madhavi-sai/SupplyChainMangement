...
class Firestore{
  ...
  User? user = FirebaseAuth.instance.currentUser;
    DocumentSnapshot documentSnapshot = await FirebaseFirestore.instance.collection('AllUsers').doc(user?.uid).get();
    return documentSnapshot.get('name');
  }


  Future<Map<String, dynamic>> getAllUsersInDomains(String domain) async {
    DocumentSnapshot documentSnapshot = await FirebaseFirestore.instance.collection('DomainWiseUsers').doc(domain).get();
    Map<String, dynamic> data = documentSnapshot.data() as Map<String, dynamic>;
    return data;
  }

  Future<void> AddUsersInDomaines(String user_id, String domain, String username) async{
    CollectionReference users = FirebaseFirestore.instance.collection('DomainWiseUsers');
  ...
  try {
    ...
    for (QueryDocumentSnapshot document in querySnapshot.docs) {
      ...
      if(document.id == domain){
        users.doc(domain).update({user_id: username})
    ...
    users.doc(domain).set({user_id: username})
          .then((value) => print("User email updated successfully!"))
          .catchError((error) => print("Failed to update user email: $error"));
