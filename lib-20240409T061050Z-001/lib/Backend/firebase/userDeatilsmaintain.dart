...
class UserDetailsTable {
  ...
  Future<String> generateUserId(String userName) async{
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('AllUsers').get();
    ...
    String baseUserId = userName.toLowerCase().replaceAll(' ', '') + getRandomNumberString(3);
  ...
  Future<void> addUser(String authid, String name, String email, int location,
      int phoneno, String domain, String village, context, String user_id) async {
  ...
  Future<Map<String, dynamic>?> fetchprofileurl({String Auth_id = ''}) async {
    String user = Auth_id == '' ? FirebaseAuth.instance.currentUser!.uid : Auth_id;

    CollectionReference users =
        FirebaseFirestore.instance.collection('AllUsers');
    CollectionReference profileurlRef =
        users.doc(user).collection('Profile');
    try {
      final profilesnapshot = await profileurlRef.doc('Profile').get();
      // print(profilesnapshot.data());
      // final url = profilesnapshot.data();
